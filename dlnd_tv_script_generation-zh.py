...
def create_lookup_tables(text):
    ...
    """
    chars = set(text)
    vocab_to_int = {c:i for i,c in enumerate(chars)}
...
def get_batches(int_text, batch_size, seq_length):
    ...
    x_text = np.array(int_text[:n_batches*batch_size*seq_length])
    y_text = np.array(int_text[1:n_batches*batch_size*seq_length+1])
    y_text[-1] = x_text[0]
    
    x = np.split(x_text.reshape(batch_size, -1), n_batches, 1)
    y = np.split(y_text.reshape(batch_size, -1), n_batches, 1)
    ...
    """
    # get x,y in the right order
    for i in range(n_batches):
        x = text_batches[:, i *seq_length : (i+1)*seq_length]
        if (i+1) * seq_length +1 < text_batches.shape[1]:
            y = text_batches[:, i *seq_length + 1: (i+1)*seq_length + 1]
        else:
            y = np.concatenate((text_batches[:, i *seq_length + 1: (i+1)*seq_length], np.zeros((text_batches.shape[0],1))),axis = 1)
        print(y)
        print('\n')
        out_text[i][0] = x
        out_text[i][1] = y
